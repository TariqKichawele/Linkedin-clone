...
export const sendConnectionRequest = async (req, res) => {
    try {
        ...
        if(existingRequest) {
            return res.status(400).json({ message: "You have already sent a pending connection request to this user." });
        }
...
export const acceptConnectionRequest = async (req, res) => {
    try {
        ...
        await User.findByIdAndUpdate(request.sender._id, { $addToSet: { connections: recipientId }})
        await User.findByIdAndUpdate(recipientId, { $addToSet: { connections: request.sender._id }})
        ...
        try {
            ...
            console.error('Error sending connection accepted mail: ' + error.message)
        }
    } catch (error) {
        res.status(500).json({ message: 'Server error'})
    }
